...
export class VpcLookupStack extends Stack {
  ...
  private async initialize(envConfig: EnvironmentConfig): Promise<void> {
    const vpcTagName = `${envConfig.stageTagName}`;
    ...
    const filterSubnets = (suffix: string) =>
      subnetsResponse.Subnets?.filter((subnet) =>
        subnet.Tags?.some(
          (tag) =>
            tag.Key === "Name" &&
            tag.Value?.startsWith(`${envConfig.stageTagName}-${suffix}`),
        ),
      )
    ...
    const findSecurityGroup = (name: string): string => {
      const sg = securityGroupsResponse.SecurityGroups?.find(
        (sg) => sg.GroupName === name,
      );
      const securityGroupId = sg?.GroupId ?? "";

      console.log(`Security group id for ${name}: ${securityGroupId}`);
