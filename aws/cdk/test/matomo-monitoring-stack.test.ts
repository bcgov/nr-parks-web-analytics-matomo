...
describe('MatomoMonitoringStack', () => {
  it('should create CloudWatch alarms and SNS topic for notifications', () => {
    ...
    const loadBalancer = new elbv2.ApplicationLoadBalancer(testStack, 'TestLoadBalancer', {
      vpc,
      internetFacing: true,
    });
    
    const targetGroup = new elbv2.ApplicationTargetGroup(testStack, 'TestTargetGroup', {
      vpc,
      port: 80,
    ...
    const taskDefinition = new ecs.FargateTaskDefinition(testStack, 'TestTaskDef', {
      memoryLimitMiB: 512,
      cpu: 256,
    });
